...
def visualize(save_path, origin, result, epoch_num=0, cond_frame_num=10, skip_pic_num=1, save_pic_num=8, save_pic_row=True, save_gif=True, save_gif_grid=True):
    ...
    # 确认文件夹存在
    epoch_save_path = os.path.join(save_path, f"epoch{epoch_num}")
    ...
    if save_pic_row:
        ...
        for i in range(save_pic_num):
            ...
            media.write_image(os.path.join(save_pic_row_path, f"pic_row_epoch{epoch_num}_sample{i}.png"), two_video.squeeze().numpy())
    ...
    if save_gif_grid:
        ...
        media.write_video(os.path.join(save_gif_grid_path, f"save_gif_grid_epoch{epoch_num}_sample.gif"), final_grids.numpy(), codec='gif', fps=20)
